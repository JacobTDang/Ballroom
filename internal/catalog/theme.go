...
// MosaicBannerScaled renders the BALLROOM title art as a scattered
// multi-color mosaic (each small block of characters gets its own color
// from the palette, diagonally offset per row) instead of a smooth
// per-row gradient — closer to light scattering off a disco ball than a
// sunset gradient. Only the letter strokes are colored; true blank
// space stays blank, so the silhouette stays unambiguous no matter how
// many colors are cycling through it. phase shifts the mosaic pattern;
// incrementing it on a timer (see internal/tui's tick handling)
// animates a shimmer across the letters — pass phase=0 for a static
// render. scale repeats each source column that many times, a nearest-
// neighbor pixel-art scale-up: scale 1 fits the banner into a narrow
// column (such as beside the disco ball in the two-column dashboard),
// larger scales read as genuinely bigger art, not just the same art
