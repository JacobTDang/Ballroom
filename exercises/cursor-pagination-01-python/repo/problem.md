...
API: `CursorStore(records)`, `.insert(record)`, `.list(page_size, page_token="") -> (items, next_page_token)` — raises `ValueError` on an invalid/tampered token or a page-size change mid-walk.
