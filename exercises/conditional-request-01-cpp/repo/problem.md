...
API: `struct GetResult { int status; std::string etag; std::string body; }`, `struct PutResult { int status; std::string etag; }`; `class ConditionalStore { GetResult Get(const std::string& key, const std::string& if_none_match); PutResult Put(const std::string& key, const std::string& body, const std::string& if_match); void Delete(const std::string& key); }`. An empty string means "unset" for `if_none_match`/`if_match` — real etags are always non-empty.
