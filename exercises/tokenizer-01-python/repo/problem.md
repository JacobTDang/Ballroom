...
API: `tokenize(input) -> list[tuple[kind, text, pos]]`, raising `ValueError` (message naming the position) on bad input. Kinds: "number", "ident", "op", "lparen", "rparen". No `re` — write the loop.
