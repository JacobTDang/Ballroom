...
API: `type GetResult struct { Status int; ETag string; Body string }`, `type PutResult struct { Status int; ETag string }`, `NewConditionalStore() *ConditionalStore`, `Get(key, ifNoneMatch string) GetResult`, `Put(key, body, ifMatch string) PutResult`, `Delete(key string)`. Empty string means "unset" for `ifNoneMatch`/`ifMatch` — real etags are always non-empty.
