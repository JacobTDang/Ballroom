...
API: `type BeginState int` (`BeginExecute`, `BeginInFlight`, `BeginReplay`), `type BeginResult struct { State BeginState; Response string }` (`Response` only meaningful when `State == BeginReplay`), `NewIdempotencyStore(ttlMillis int64) *IdempotencyStore`, `BeginAt(key, fingerprint string, nowMillis int64) (BeginResult, error)`, `CompleteAt(key, response string, nowMillis int64) error`. Both return a non-nil `error` on a fingerprint conflict or an invalid `Complete`.
