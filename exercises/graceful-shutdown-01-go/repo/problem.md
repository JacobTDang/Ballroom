...
API: `NewServer(workers int, handle func(int)) *Server`, `Submit(v int) bool`, `Stop()`. The jobs queue may be a large buffered channel (the tests submit at most 200 at once). Tests run with `-race`.
