...
API: `struct Record { long id; std::string name; }`; `class CursorStore { CursorStore(std::vector<Record> records); bool Insert(const Record& r, std::string* err); bool List(int page_size, const std::string& page_token, std::vector<Record>* items, std::string* next_page_token, std::string* err); }` — `List`/`Insert` return `false` and fill `*err` on an invalid/tampered token, a page-size change mid-walk, or a duplicate id.
