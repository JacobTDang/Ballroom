...
API: `void RunLimited(const std::vector<std::function<void()>>& tasks, int limit)`. C++17 has no std semaphore — build one from a mutex and condition variable. Compiled with `-fsanitize=thread`.
