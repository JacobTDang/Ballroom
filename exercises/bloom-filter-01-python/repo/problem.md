...
API: `BloomFilter(bits, hashes)`, `.add(key)`, `.might_contain(key) -> bool`. Derive your base hashes by hand (e.g. FNV-1a) or from `hashlib` — but not from a set/dict, which would defeat the exercise.
