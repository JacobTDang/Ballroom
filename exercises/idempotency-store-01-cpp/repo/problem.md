...
API: `enum class BeginState { kExecute, kInFlight, kReplay }`, `struct BeginResult { BeginState state; std::string response; }` (`response` only meaningful when `state == kReplay`); `class IdempotencyStore { IdempotencyStore(long ttl_ms); bool BeginAt(const std::string& key, const std::string& fingerprint, long now_ms, BeginResult* out, std::string* err); bool CompleteAt(const std::string& key, const std::string& response, long now_ms, std::string* err); }` — both return `false` and fill `*err` on a fingerprint conflict or an invalid `Complete`.
