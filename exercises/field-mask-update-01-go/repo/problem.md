...
`target`, in place. Values are plain nested objects: `map[string]any`
where each entry is either a nested `map[string]any` (an object) or a
`string` (a leaf).
...
API: `Update(target, source map[string]any, mask []string) error` — mutates `target` in place; returns a non-nil error (naming the offending path) on an empty mask or an unresolvable intermediate segment.
