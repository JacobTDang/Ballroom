...
API: `NewBarrier(n int) *Barrier`, `(*Barrier).Wait()`. Don't reach for sync.WaitGroup — it's not reusable mid-flight; a mutex + condition variable with a generation counter is the classic build. Tests run with `-race`.
